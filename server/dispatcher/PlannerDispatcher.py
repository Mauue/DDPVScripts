...
class PlannerDispatcher:
    ...
    def _new_requirement(self, topology, out, requirement, addition_hops=-1):
        p = Planner()
        self._set_state("parse topology")
        p.add_topologies(topology)
        write_topology(topology, out)
        self._set_state("build topology")
        p.build()
        self._set_state("build requirement")
...
def write_topology(edges, out):
    with open(os.path.join(out, "1.topology"), mode="w", encoding="utf-8") as f:
        for edge in edges:
